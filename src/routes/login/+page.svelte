...
<div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
  <div class="sm:mx-auto sm:w-full sm:max-w-sm">
    <img src={asset("/resources/image_style/The_International_2025_aegis_allmode.png")} alt="Organizatino" class="mx-auto w-auto" />
  ...
  <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
    <form method="POST" class="space-y-6" use:enhance>
      <div>
        <label for="email" class="block text-sm/6 font-medium text-black-800">Username</label>
        <div class="mt-2">
          <input id="email" type="text" name="username" required autocomplete="email" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
        </div>
      </div>
    ...
    <p class="mt-10 text-center text-sm/6 text-gray-400">
      <a href="/signup" class="font-semibold text-indigo-400 hover:text-indigo-300">Sign Up here</a>
    </p>
  </div>
  <div>
    <p>{form?.message ?? ""}</p>
